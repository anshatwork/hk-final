...
def search(input_keyword, brand,category,page):
    ...
    if brand:
        brand_filter = {
            "script": {
                "script": {
                    "source": "doc['br_nm'].value.toLowerCase() == params.brand.toLowerCase()",
                    "params": {
                        "brand": brand
                    }
                }
        ...
        query["query"]["bool"]["filter"].append(brand_filter)

    if category:
        category_filter = {
            "script": {
                "script": {
                    "source": "doc['secondary_category'].value.toLowerCase() == params.category.toLowerCase()",
                    "params": {
                        "category": category
                    }
                }
        ...
        query["query"]["bool"]["filter"].append(category_filter)

    print('ran uqery with page no ' + str(page))
...
def search_count(input_keyword,brand,category):
...
def main():
    ...
    if st.button("Search"):
        ...
        print(st.session_state.search_query)


        st.session_state.page_number = 0  
        st.session_state.total = len(search_count(st.session_state.search_query,st.session_state.brand,st.session_state.category))
        ...
        if st.session_state.flag :
            print('total fuzzy search ' + str(st.session_state.total))
        else : print('total keyword search ' + str(st.session_state.total))
    ...
    if st.session_state.search_query:
        ...
        if st.session_state.page_number > last_page -3:
            ...
            if not st.session_state.flag :
                results = search(st.session_state.search_query,st.session_state.brand,st.session_state.category,st.session_state.page_number)
            ...
            if len(results) < N:
                ...
                # flag is triggered if the number of search results from Keyword search is not sufficient and a fuzzy search is ran
                else : printllm(data)
                ...
                if st.session_state.flag :
                    ...
                    # With the Updated Keyword filter query no need to filter out brands and categories manually
                    printllm(data)
